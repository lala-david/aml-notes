...
```mermaid
flowchart TB
    AGREE[2023-11 DOJ 합의 $4.3B] --> MON[외부 모니터 5년 상주]
    AGREE --> EXEC[CZ 사임 + 실형]
    AGREE --> ORG[조직 전면 재편]
    ORG --> HUB[5개 지역 hub<br/>두바이·런던·싱가포르·아부다비·바하마]
    ORG --> CCO[각 hub CCO 신규 채용]
    MON --> REPORT[분기별 DOJ 보고서]
    MON --> CHANGE[조직·시스템 변경 사전 승인]
    style AGREE fill:#fee2e2
    style CHANGE fill:#fed7aa
```
