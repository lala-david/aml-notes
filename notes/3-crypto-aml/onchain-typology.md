...
> 가상자산 자금세탁의 **실제 패턴**. 어떻게 layering 하는가. 이 글을 읽고 나면 KYT 시스템이 탐지하려는 9개 패턴이 각각 **어떤 원리로 작동**하고 **왜 탐지가 어려운지** 설명할 수 있게 됩니다. 마지막 업데이트: 2026-08-04.
...
**CMLN (Chinese Money Laundering Network)** 이 이 영역에서 부상 — 2025년 **$16.1B 처리, 활성 wallet 1,799개**. 북한 자금까지 세탁 파트너 역할.
...
- 2025년 **$16.1B 처리**, 활성 wallet 1,799개
- 북한·러시아 자금까지 이용
...
- **러시아 A7A5 stablecoin**: 2025-02 발행 후 **10개월간 $93.3B** 거래(Chainalysis 2026) — 제재 회피 인프라로 급성장했다가 **제재로 무너진 사례**
  - EU 19차 패키지(2025-10)가 **암호자산에 명시적 거래금지를 건 첫 사례**로 A7A5를 지정, 20차 패키지(2026-05-24 발효)가 러시아 암호자산 사업자 거래를 포괄 금지
  - 누적 $102B·약 251,000건까지 갔으나 2026-06 일평균 거래액 $24.3M — **2025-07 정점 대비 96% 감소**(Elliptic)
  - 시사점: 제재가 온체인 인프라에 **실제로 작동한** 드문 사례. "스테이블코인은 제재를 우회한다"는 통념의 반례로 쓸 수 있다
...
**정확도에 대한 주의**: "CIOH는 정확도 X%" 식의 단일 수치는 **인용 가능한 근거가 없습니다.** Möser & Narayanan (*Resurrecting Address Clustering in Bitcoin*, arXiv 2107.05749 / FC 2022)은 논문 초록에서 오히려 이렇게 지적합니다 — *"current techniques rely on heuristics that **have not been rigorously evaluated or optimized**"*. 이 논문의 기여는 거스름돈 식별의 **ground truth 집합을 구축**하고, 낮은 오탐률을 목표로 하는 예측 모델을 만들고, **클러스터 붕괴(cluster collapse)** 를 탐지·예방하는 방법을 제시한 것입니다.

실무 함의는 "정확도 몇 %"가 아니라 **cluster collapse 위험**입니다. 휴리스틱이 잘못 병합하면 서로 무관한 엔티티가 한 클러스터로 뭉치고, 그 위에 올린 위험 점수가 통째로 오염됩니다. 벤더 PoC에서 물어야 할 것은 정확도 수치가 아니라 "붕괴를 어떻게 탐지하고 되돌리는가"입니다.
