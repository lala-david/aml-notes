...
### 9의무 별 담당자·시스템·SLA·증빙 (Upbit 스타일)

| 의무 | 담당 조직 | 핵심 시스템 | SLA | 검사관 요구 증빙 |
|---|---|---|---|---|
| 신고 | 법무·AMLO | FIU 제출문서 + ISMS 인증서 | 3년 갱신 | 수리증 + 갱신 준비 로그 |
| KYC | 온보딩팀 | PASS/NICE + ARGOS OCR | 실시간 (~분) | 온보딩 스냅샷 + 실명매칭 로그 |
| CDD | AML 분석팀 | CRM risk_tier + 재실사 스케줄러 | 연 1회 refresh | 재실사 완료 타임스탬프 |
| EDD | AML + AMLO | Jira 티켓 + 자금원천 업로드 | 7일 내 결재 | 결재 로그 + 증빙 파일 |
| KYT | KYT 엔지니어링 | Chainalysis API + Kafka 스트림 | 실시간 (~초) | 알람 큐 + disposition 사유 |
| 제재 | AML 분석팀 | OFAC SDN.XML diff + fuzzy 매칭 | OFAC 업데이트 24h 내 | 재스캔 로그 |
| STR | AML 분석팀 + AMLO | FIU-TIS 포털 (수기) | 판단일 30일 내 | 접수번호 + 의심사유 서술 |
| Travel Rule | KYT 엔지니어링 | VerifyVASP + CODE 이중 연동 | 실시간 | 메시지 로그 + 거절 사유 |
| 기록 | 데이터팀 | S3 Glacier + WORM | 15년 보존 | 접근 로그 + 무결성 해시 |
| 내부통제 | AMLO + 내부감사 | 정책서 Confluence + 교육 LMS | 연 1회 교육·감사 | 이수율 + 감사 보고서 |

### VASP 유형별 실제 조직 구성

**거래소 (Upbit)**: AMLO + 분석팀 12~15명 + KYT/KYC 엔지니어 3~5명 + 법무 별도

**수탁업 (KODA)**: CCO + 소규모 컴플팀 3~5명 + **보안·키관리팀이 더 큼** (10+명)

**OTC desk**: 단일 AMLO + 분석가 1~2명 + 영업과 **방화벽** (별도 승인 체계)

**NFT 마켓 (K-pop NFT 등)**: 대부분 **외주 컴플라이언스** (법무법인에 월 수천만원 위탁)

### Binance CZ 사례 이후 임원 개인책임 강화

- **CZ 4개월 실형** (2024-05) + 개인 $50M 벌금 → 글로벌 VASP 임원이 **D&O 보험 개인 추가**로 구매
- 한국 AMLO도 2025년 이후 **개인 책임보험** 가입 트렌드 (연 수백만원)
- **AMLO 결재 로그**를 위변조 불가능한 WORM 스토리지에 남기는 SOP 확산 → 개인 변호 자료

### 자주 나오는 오해

- **"9가지 다 있으면 통과"** — 검사관은 "있지만 작동하는가"를 봄. STR 제출 0건 + 수상한 거래 1건이면 즉시 지적.
- **"수탁업은 AML 의무 약하다"** — 분리보관 실패가 훨씬 치명적. Celsius·FTX 파산 주원인이 수탁 실패.
- **"Travel Rule은 100만원 이상만"** — 분할송금 회피 감지하기 위해 90만원대 반복도 EDD.
- **"AMLO는 CEO가 겸임 가능"** — 한국 FIU는 CEO·AMLO 겸임 허용하지만 **실권 분리** 증명 필요. 대주주 AMLO는 지적 대상.

### 룰 예시 — RBA Risk Score 의사코드

```python
def compute_customer_risk(c):
    score = 0
    if c.pep: score += 40
    if c.country in FATF_GREY: score += 30
    if c.country in FATF_BLACK: score += 80
    if c.onboarding_channel == "NON_FACE_TO_FACE": score += 10
    if c.avg_monthly_volume_krw > 500_000_000: score += 20
    if c.occupation in ["CASH_INTENSIVE", "CRYPTO_MINER"]: score += 15
    if c.external_wallets_registered >= 10: score += 10
    return min(score, 100)  # Tier L1(<30) / L2(30~69) / L3(70+)
```

### 한국 특수 현실

- **대주주 자격심사(2026-01)**: 신고 수리 후 **주주 변동 시 재심사**. M&A 계약서 "FIU 승인 조건" 조항 필수.
- **DAXA 공동 상장 심사**: 5사 공동 기준. 한 거래소 거부 = 사실상 전체 거부.
- **수탁업 차별화**: KODA·KDAC 등은 **은행 지주사 산하**라 은행 수준 감독 병행. 독립 수탁사(Bitgo Korea 등)는 문턱 더 높음.
