...
> FATF Recommendation 16의 가상자산 적용. VASP 운영 난이도 **1순위** 테마. 이 글을 읽고 나면 Travel Rule이 왜 단순 규제가 아니라 **글로벌 인프라 문제**인지, 그리고 왜 한국이 두 개의 솔루션을 동시에 운영하는지 이해하게 됩니다. 마지막 업데이트: 2026-08-04.
...
- 금액 기준은 **두 유형**으로 갈림 — 의무 자체에 붙는 관할(미국 $3,000 · 한국 100만원)과, 의무는 전건이고 금액이 **정보의 깊이만 나누는** 관할(EU·영국·일본·홍콩·싱가포르). 한 칸에 "임계 얼마"로 적으면 후자가 전부 틀림
...
![Travel Rule의 금액 기준은 두 종류 — 의무를 발동시키는 A형(미국·한국)과 정보 항목만 늘리는 B형(EU·영국·일본·홍콩·싱가포르)](../../charts/output/travel_rule_thresholds.svg)


### 이 표를 어떻게 읽어야 하나

**"임계금액"을 한 칸에 적으면 대부분의 관할이 틀립니다.** 관할은 두 유형으로 갈립니다.

- **A형 — 의무 자체에 금액이 붙는다.** 기준 미만이면 Travel Rule 대상이 아예 아닙니다. 미국·한국.
- **B형 — 의무에는 금액이 없고, 금액은 정보의 깊이를 나눈다.** 1원짜리 이체에도 의무는 발생하고, 기준을 넘으면 넘겨야 할 항목이 늘어납니다. EU·영국·일본·홍콩·싱가포르.

B형 관할에 A형처럼 "임계 얼마"라고 적으면 **기준 미만 거래를 대상 외로 처리하는 설계 오류**로 이어집니다. 실제로 이 문서가 그렇게 적고 있었습니다.

| 관할 | 유형 | 의무 발동 | 정보 심화 기준 | 근거 조문 |
|---|---|---|---|---|
| 🌐 FATF 권고 | — | 임계 없음 | USD/EUR 1,000 | INR.15 §7(b) |
| 🇺🇸 **미국** | **A** | **$3,000 이상** | — | 31 CFR 1010.410(f) |
| 🇰🇷 **한국** | **A** | **100만원 이상** | — | 특금법 시행령 §10의10 |
| 🇪🇺 EU | B | 금액 무관 전건 | (EUR 1,000은 별개 의무) | TFR 제14조 |
| 🇬🇧 영국 | B | 금액 무관 전건 | £800 **이상** | MLR 2017 reg 64C |
| 🇯🇵 일본 | B | 금액 무관 전건 | 없음 | 범수법 제10조의5 |
| 🇭🇰 홍콩 | B | 금액 무관 전건 | HK$8,000 **이상** | AMLO 부칙2 §13A |
| 🇸🇬 싱가포르 | B | 금액 무관 전건 | S$1,500 **초과** + 신원확인 | MAS PSN02 §13 |
| 🇦🇺 호주 | B | 금액 무관 전건 | (AML/CTF Rules 위임) | AML/CTF법 §66A |

여덟 관할 전부 근거 조문 원문으로 확인했습니다.

> ⚠️ **자주 인용되는 두 숫자는 Travel Rule 기준이 아닙니다.**
>
> - **일본 10만엔** — 범수법 *시행령* 제7조1항1호ヤ의 **특정거래(特定取引)** 정의이며 **고객확인(CDD)** 을 발동시키는 기준입니다. 통지의무를 정한 법 제10조의5, 그 위임을 받은 시행규칙 제31조의7 어디에도 금액 조건이 없습니다(법률·정령·성령 3층위 원문 확인). 덧붙여 문언은 「十万円を**超える**」 초과이지 이상이 아닙니다.
> - **EU EUR 1,000** — 자기수탁(unhosted) 주소의 소유·통제를 **평가할 의무**를 발동시키는 별개 기준입니다. 정보 동반 의무 자체에는 금액이 없습니다.

**경계값은 관할마다 다릅니다.** 뭉뚱그리면 셋 다 틀립니다.

| 관할 | 문언 | 경계값이 속하는 구간 |
|---|---|---|
| 홍콩 | *not less than* HK$8,000 | 정확히 8,000 → **상위** |
| 영국 | *equals or exceeds* £800 | 정확히 800 → **상위** |
| 싱가포르 | *below or equal to* / *exceeds* S$1,500 | 정확히 1,500 → **하위** |
| 일본 | 十万円を*超える* (CDD 기준) | 정확히 10만엔 → **대상 아님** |

**싱가포르만 성격이 다릅니다.** 다른 B형은 금액이 정보 항목만 나누는데, 싱가포르는 S$1,500 초과 시 *"shall identify … and verify the value transfer originator's identity"* 로 **신원확인·검증 의무**가 함께 붙습니다. 정보 심화와 CDD 강화가 같은 문턱에 있습니다.

**호주는 한 층 더 남아 있습니다.** §66A·§64 본문에 금액이 없는 것은 확인했지만, 전달할 정보 *항목*은 AML/CTF Rules에 위임돼 있습니다. 일본의 주무성령, 영국의 reg 64C(5)·(6)과 같은 위치이므로 Rules를 봐야 "정보 심화 기준 없음"을 확정할 수 있습니다.

### 실무 포인트

한국 사업자가 EU·영국 카운터파티와 거래할 때는 **금액 무관 전건 대응**이 기본값입니다. 1유로짜리 거래여도 의무는 발생합니다. 이게 한국 거래소가 EU 고객 온보딩에 미적거리는 구조적 이유 중 하나.

룰 엔진을 만들 때는 **B형 관할에 금액 게이트를 걸면 안 됩니다.** 게이트는 "의무 발동"이 아니라 "어떤 필드를 채울 것인가"에 걸어야 합니다. 두 유형을 한 코드 경로로 처리하려다 나는 사고가 실제로 많습니다.
