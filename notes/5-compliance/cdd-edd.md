...
## 8. RBA (Risk-Based Approach) — 고객 위험 점수 산정법

### 8.1 왜 점수화가 필요한가

FATF R.1·한국 특금법 §5의2 모두 **"위험에 비례해 통제 강도 조정"**을 요구. 이를 운영화하려면 **정량적 위험 점수**가 필요. 질적 판단만으로는:
- 검사 시 **일관성 증명 불가**
- 분석가 간 편차 (같은 고객 다르게 판단)
- 룰 자동화 불가
...
**가중치 합 = 100** (또는 1.0 정규화)

### 8.3 각 Factor 점수표

#### Customer Factor (가중 25~30)

| 속성 | 점수 |
|---|---|
| PEP 본인 | 30 |
| PEP 가족·측근 | 20 |
| 고위험 직업(카지노·총기·암호화폐 거래소) | 15 |
| 일반 전문직 (의사·변호사 등) | 5 |
| 일반 회사원·공무원 | 0 |
| 학생·주부 | 0 (단 고액 거래 시 가중) |

#### Geography Factor (가중 20~25)

| 국가 | 점수 |
|---|---|
| FATF Black List (북한·이란) | 40 |
| FATF Grey List (14개국, 2026-04 기준) | 25 |
| OFAC Comprehensive (쿠바·시리아·베네수엘라 등) | 35 |
| Tax Haven (케이맨·BVI 등) | 15 |
| 한국·미국·EU·일본·영국 | 0 |
| 기타 | 5 |

#### Product Factor (가중 15~20)

| 서비스 | 점수 |
|---|---|
| Privacy coin 거래 (Monero·Zcash 등) | 30 |
| Mixer·Tumbler 접촉 이력 | 25 |
| Self-hosted wallet 대용량 입출금 | 15 |
| Staking·LST | 10 |
| 기본 spot trading | 0 |
| KRW 입출금만 | 0 |

#### Channel Factor (가중 10~15)
...
#### Behavior Factor (가중 15~20)

| 패턴 | 점수 |
|---|---|
| Smurfing·Structuring 의심 | 40 |
| 24h 내 입·출금 반복 (pass-through) | 30 |
| 새벽 고속 거래 | 20 |
| 정상 | 0 |

### 8.4 등급 매핑

| Total Score | 등급 | CDD/EDD |
|---|---|---|
| 0~29 | **Low** | 간소 CDD (문서 축소) |
| 30~59 | **Medium** | 표준 CDD |
| 60~79 | **High** | EDD 자동 트리거 |
| 80+ | **Critical** | EDD + 수동 승인 + AMLO 검토 |

### 8.5 워크드 예시

**고객 A**: 한국인, 의사, 비대면 KYC, 정상 거래, Staking 관심
- Customer: 전문직 5 × 0.25 = 1.25
- Geography: 한국 0 × 0.20 = 0
- Product: Staking 10 × 0.20 = 2.0
- Channel: Video KYC 15 × 0.10 = 1.5
- Behavior: 정상 0 × 0.25 = 0

Total: 4.75 / 100 → **Low**

**고객 B**: 아랍에미리트 거주, 건설업자, 비대면 KYC, Privacy coin 관심, 24h 입출금 반복
- Customer: 고위험직업 15 × 0.25 = 3.75
- Geography: 기타 5 × 0.20 = 1.0
- Product: Privacy coin 30 × 0.20 = 6.0
- Channel: Video KYC 15 × 0.10 = 1.5
- Behavior: pass-through 30 × 0.25 = 7.5

Total: 19.75 → Low? **아니. 이 스케일에선 × 4 하는 정규화 적용**

일반 운영: 각 factor에 0~100 직접 부여 + 가중치 합 = 1.0 형식:

```
score = 0.25 × 15 + 0.20 × 5 + 0.20 × 30 + 0.10 × 15 + 0.25 × 30
      = 3.75 + 1.0 + 6.0 + 1.5 + 7.5
      = 19.75 + 지수 조정
```

**현장 공식**: 각 factor 0~100 점수 × 가중치 → 합산. 고객 B의 경우 (15·5·30·15·30 각 가중) = 평균 19~25. 하지만 **고점(30)이 있으면 상한을 끌어올리는 보정** 규칙 적용:

```python
def calibrate(raw_score, factors):
    if max(factors) >= 25:
        raw_score = max(raw_score, 50)  # Medium 최소 보장
    if max(factors) >= 35:
        raw_score = max(raw_score, 70)  # High 강제
    return min(raw_score, 100)
```

이 보정을 적용하면 고객 B는 Privacy coin 30점 + pass-through 30점 → **보정 후 50+** → Medium 이상.
...
검사 시 FIU·FSS는 다음을 요구:
- 점수 산정 **공식 문서화** (엑셀·pseudocode 가능)
- 각 factor **가중치 결정 근거**
- 최근 1년 **등급 분포** (Low/Medium/High/Critical %)
- 재평가 이력 샘플
...
이 분포에서 **Critical 비중 과도(5%+)면 고객 프로파일 점검 필요**, **Low 비중 과도(90%+)면 RBA 운영 부실 의심**.
